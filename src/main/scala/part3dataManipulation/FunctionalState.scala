...
object FunctionalState {
  ...
  // State — это структура, обеспечивающая функциональный подход к обработке состояния приложения.
  // State[S, A] в основном это функция S => (S, A), где S тип, представляющий ваше состояние, и A результат, который производит функция.
  // В дополнение к возврату результата типа A функция возвращает новое S значение, которое является обновленным состоянием.
  ...
  // transform shopping cart to another shopping cart and the total amount at that point
